...
CREATE TABLE \`turnos\` (
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-[#FFFFFF] font-sans flex flex-col selection:bg-blue-brand/30 selection:text-white-brand relative overflow-hidden">
      ...
      {activePortalView === 'admin' && (
        <main className="flex-1 overflow-y-auto p-6 max-w-7xl mx-auto w-full relative z-10 space-y-6 text-left">
          {/* Header row */}
          <div className="bg-gradient-to-r from-[#141B25] to-[#1A2534] rounded-2xl p-6 border border-cyan-500/25 shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1.5">
            ...
            <div className="flex gap-2.5 font-mono text-xs">
              <div className="bg-black/30 border border-gray-800 rounded-xl px-4 py-2 text-center">
                <span className="text-[10px] text-gray-500 block uppercase">Billetera Carlos (80%)</span>
                <span className="text-orange-400 font-bold">
                  {walletSaldo.toLocaleString('es-AR', { style: 'currency', currency: 'ARS', minimumFractionDigits: 0 })}
                </span>
              </div>
              <div className="bg-black/30 border border-gray-800 rounded-xl px-4 py-2 text-center">
                <span className="text-[10px] text-gray-500 block uppercase">SaaS Comisiones (20%)</span>
      ...
      {(activePortalView === 'sandbox' || activePortalView === 'repartidor') && (
        ...
        {activeWorkspaceTab !== 'mockups' && (
          ...
          <div className="relative w-full max-w-[340px] aspect-[9/18.5] bg-black rounded-[40px] p-2.5 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.9)] border-4 border-gray-800 ring-12 ring-gray-900/50 flex flex-col overflow-hidden">
                ...
                {phoneScreen === 'perfil' && (
                  <div className="flex-1 p-4 flex flex-col gap-4 text-center">
                    ...
                    <div className="space-y-2 text-left">
                      <span className="text-[10px] font-bold text-gray-400 uppercase block">Logros y Reconocimientos</span>
                      
                      <div className="bg-orange-500/10 border border-orange-500/20 p-2.5 rounded-xl flex gap-2.5 items-center">
                        <Award className="w-6 h-6 text-orange-500 shrink-0" />
                        <div>
                          <span className="font-bold text-white text-[10px] block leading-none">Relámpago Nocturno</span>
                          <span className="text-[9px] text-gray-400 block mt-0.5leading-normal">Más de 100 entregas durante turnos de lluvia con satisfacción perfecta.</span>
                        </div>
                      </div>
                    </div>
